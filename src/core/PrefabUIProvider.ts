...
export class PrefabUIProvider {
    ...
    public async showPrefabUI(): Promise<void> {
        ...
        // 设置Webview内容
        this._setupWebviewContent(panel, activeEditor);
    }

    private _setupWebviewContent(panel: vscode.WebviewPanel, activeEditor: vscode.TextEditor): void {
  ...
  <script type="module" nonce="${nonce}" src="${panel.webview.asWebviewUri(webviewUri)}"></script>
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            if (message.command === 'getPrefabData') {
                ...
                await this._handleJumpToNode(message, activeEditor);
            }
        });
    ...
    private async _handleJumpToNode(message: any, activeEditor: vscode.TextEditor): Promise<void> {
        try {
            ...
            if (document) {
                ...
                if (nodeIndex >= 0) {
                    // 使用原来的跳转逻辑
                    const { dealEditor } = await import('./dealPrefabFile');
