...
// 拖拽调整面板宽度
const nodeTreeWidth = ref(0) // 将在 onMounted 中计算30%宽度
...
onMounted(() => {
  ...
  // 计算节点树默认宽度为30%
  const updateNodeTreeWidth = () => {
    const totalWidth = window.innerWidth
    nodeTreeWidth.value = Math.floor(totalWidth * 0.3)
    console.log('PrefabUI: 设置节点树宽度为30%:', nodeTreeWidth.value)
  }
  ...
  // 监听窗口大小变化
  window.addEventListener('resize', updateNodeTreeWidth)
